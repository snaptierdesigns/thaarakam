...
export default function CartClient({ settings }: CartClientProps) {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    const razorpayKey = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || 'rzp_live_TPJiMn4Qvczv7y';
  ...
  if (confirmedOrder) {
    return (
      <div className="mx-auto max-w-[720px] px-4 py-8 sm:px-6 flex flex-col items-center gap-6">
        
        {/* Primary Top Action Button */}
        <div className="w-full flex justify-between items-center">
          <Link
            href="/shop"
            className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-secondary hover:text-foreground transition-colors"
          >
        ...
        <div className="w-full rounded-3xl border border-border bg-background p-6 sm:p-8 shadow-sm flex flex-col items-center text-center gap-5 relative overflow-hidden">
          
          {/* Top Decorative Sparkles */}
          <div className="flex items-center justify-center gap-1.5 text-secondary/60">
          ...
          {/* Checkmark Circle */}
          <div className="rounded-full bg-foreground text-background p-2.5 my-0.5 shadow-sm">
            <CheckCircle2 className="h-6 w-6 stroke-[2.5]" />
          </div>

          {/* Headline & Subheadline */}
          <div className="flex flex-col gap-1">
            <h2 className="text-base sm:text-lg font-bold uppercase tracking-wider text-foreground">
          ...
          <div className="inline-flex items-center gap-2 rounded-xl border border-border bg-border/10 px-4 py-2.5 text-xs font-semibold text-foreground tracking-wide">
          ...
          <div className="w-full rounded-2xl border border-border bg-border/5 p-4 sm:p-5 grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
            
            {/* Dispatch Time */}
            <div className="flex flex-col items-center text-center justify-between gap-2.5 border-b md:border-b-0 md:border-r border-border pb-4 md:pb-0 md:pr-4">
              <div className="rounded-full bg-background border border-border p-2.5 shadow-xs">
                <Calendar className="h-4 w-4 text-foreground stroke-[1.5]" />
              </div>
              <span className="text-[11px] font-bold uppercase tracking-wider text-foreground">
                DISPATCH TIME
              </span>
              <div className="w-12 border-t border-dotted border-border/80"></div>
              <div className="w-full rounded-lg bg-border/20 py-2 text-xs font-bold uppercase tracking-wider text-foreground">
                UP TO 3 <span className="text-[9px] font-normal block text-secondary">WORKING DAYS</span>
              </div>
            </div>

            {/* Delivery Time (After Dispatch) */}
            <div className="flex flex-col items-center text-center justify-between gap-2.5">
              <div className="rounded-full bg-background border border-border p-2.5 shadow-xs">
                <Truck className="h-4 w-4 text-foreground stroke-[1.5]" />
              </div>
              <span className="text-[11px] font-bold uppercase tracking-wider text-foreground">
                DELIVERY TIME (AFTER DISPATCH)
              </span>
              ...
              <div className="grid grid-cols-2 gap-2 w-full">
                <div className="flex flex-col items-center justify-center rounded-lg bg-border/20 py-2 px-1">
                  <span className="text-[8px] font-bold uppercase tracking-wider text-secondary">
                    📍 INSIDE KERALA
                  </span>
                  <span className="text-xs font-bold text-foreground">3–5</span>
                  <span className="text-[8px] text-secondary uppercase font-medium">WORKING DAYS</span>
                </div>
          ...
          <div className="w-full rounded-xl border border-border bg-background p-2.5 flex items-center justify-center gap-2.5 text-xs font-semibold text-foreground">
            <span className="uppercase tracking-wider text-secondary text-[10px] font-bold">DELIVERY PARTNER</span>
            <span className="text-border">|</span>
            <div className="flex items-center gap-1.5">
              <span className="rounded bg-red-700 text-white px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider">India Post</span>
              <span className="text-xs font-medium">Speed Post</span>
            </div>
          </div>

          {/* Need Help Box */}
          <div className="w-full rounded-2xl border border-border bg-border/5 p-4 sm:p-5 flex flex-col items-center gap-3 text-center">
            <h3 className="text-xs font-bold uppercase tracking-wider text-foreground">
              NEED HELP?
            </h3>
            <p className="text-[11px] text-secondary leading-relaxed max-w-md">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 w-full mt-1">
              <Link href="/track-order" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl border border-border bg-background hover:border-foreground/40 transition-colors group">
                <Globe className="h-4 w-4 text-foreground group-hover:scale-110 transition-transform" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-foreground">WEBSITE TRACK</span>
              </Link>

              <a href="https://instagram.com/thaarakam_by_nithara" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl border border-border bg-background hover:border-foreground/40 transition-colors group">
                <Camera className="h-4 w-4 text-foreground group-hover:scale-110 transition-transform" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-foreground">INSTAGRAM DM</span>
              </a>

              <a href="https://wa.me/918921356009" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl border border-border bg-background hover:border-foreground/40 transition-colors group">
                <MessageCircle className="h-4 w-4 text-foreground group-hover:scale-110 transition-transform" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-foreground">WHATSAPP</span>
              </a>

              <Link href="/contact" className="flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl border border-border bg-background hover:border-foreground/40 transition-colors group">
                <Mail className="h-4 w-4 text-foreground group-hover:scale-110 transition-transform" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-foreground">OTHER CONTACT</span>
              </Link>
            </div>

            <p className="text-xs font-serif italic text-secondary mt-0.5">
              We will look into it.
            </p>
          </div>

          {/* Footer Signature */}
          <div className="flex items-center justify-center gap-2 text-[9px] font-bold uppercase tracking-[0.2em] text-secondary mt-1">
            <span className="text-red-400">♥</span> T H A N K &nbsp; Y O U &nbsp; F O R &nbsp; S U P P O R T I N G &nbsp; U S ! <span className="text-red-400">♥</span>
          </div>

        </div>

        {/* Dynamic Customer Order Details & Receipt Card */}
        <div className="w-full rounded-2xl border border-border bg-background p-5 sm:p-6 text-left flex flex-col gap-3.5 shadow-sm">
          <div className="flex justify-between items-center border-b border-border pb-2.5 text-xs">
          ...
          <div className="flex justify-between items-center border-t border-border pt-2.5 font-bold text-xs text-foreground">
            <span>Total Paid:</span>
            <span className="text-sm font-extrabold">₹{confirmedOrder.grand_total}</span>
          </div>
        </div>

        {/* Clear Bottom Action Buttons (CTA) */}
